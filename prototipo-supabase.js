/* FLUXO: cliente de produccion para Supabase Auth + RLS.
...
(() => {
  ...
  window.luxGoogleLogin = function() {
    ...
    if (!cfg?.url || !cfg?.publishableKey) {
      ...
      console.warn('FLUXO: LUX_SUPABASE_CONFIG no está definido. Configura SUPABASE_URL y SUPABASE_PUBLISHABLE_KEY como variables en el repositorio de GitHub.');
      return;
    }
  ...
  if (!config?.url || !config?.publishableKey) {
    console.info('FLUXO: configuracion de produccion pendiente. Se mantiene la demo local.');
    return;
  }
  ...
  function renderNavigation() {
    ...
    if (publicNav) {
      if (state.navigationContext === 'admin' && state.isStaff) {
        ...
        setStableHtml(publicNav, `public:${state.user?.id || 'guest'}:${state.role}:${state.isStaff}:${state.authStatus}`, `<button type="button" class="lux-nav-brand" onclick="window.luxHub.setScreen('home')">⚡ FLUXO</button><strong>CLASIFICACIÓN DEL CLAN</strong><span class="lux-nav-actions">${accountActions}</span>`);
      }
    }
  ...
  function requestMemberRemoval(id) {
    ...
    $('lux-remove-copy').textContent = `¿Confirmas que quieres eliminar a ${removalTarget.name} de FLUXO?`;
