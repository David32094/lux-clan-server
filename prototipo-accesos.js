...
(() => {
  ...
  function openLogin(kind='member'){
    ...
    modal.innerHTML=`<div class="lux-login-box"><button class="lux-login-close" type="button" onclick="window.luxAccess.closeLogin()">×</button><span class="hub-kicker">${isLeader?'ACCESO DEL EQUIPO':'ACCESO DE INTEGRANTE'}</span><h2>${isLeader?'Panel de líderes':'Tu cuenta del clan'}</h2><p>${isLeader?'Cada líder usa su propia cuenta. Los permisos reales se activarán al conectar la base de datos.':'Tu perfil será solo tuyo en la versión final. En esta prueba se guarda en este dispositivo.'}</p>${isLeader?`<label>CUENTA DE LÍDER<select id="lux-leader-select">${LEADERS.map(item=>`<option value="${item.id}">${item.name} · ${item.role}</option>`).join('')}</select></label><label>CLAVE LOCAL<input id="lux-login-pin" type="password" placeholder="Clave de prueba"/></label><button class="lux-login-primary" type="button" onclick="window.luxAccess.loginLeader()">ENTRAR AL PANEL</button><small>Prueba local: David usa LUX2026.</small>`:`<label>CORREO O USUARIO<input id="lux-member-email" type="text" inputmode="email" placeholder="tuusuario o correo"/></label><label>CONTRASEÑA<input id="lux-member-pass" type="password" placeholder="Mínimo 4 caracteres"/></label><button class="lux-login-primary" type="button" onclick="window.luxAccess.loginMember()">ENTRAR A MI PERFIL</button><small>Prueba local: esta cuenta no sale del teléfono.</small>`}</div>`;
    modal.hidden=false;
